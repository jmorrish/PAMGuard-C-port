...
std::string read_expected_fragments(const std::string& path) {
    ...
    if (value.empty()) {
        throw std::runtime_error("rejoin fixture did not contain a data row");
    }
    return value;
}
...
int main(int argc, char** argv) {
    if (argc < 2 || argc > 3) {
        std::cerr << "Usage: connected_region_rejoin_fixture_check <fixture.csv> [merge|split|cross]\n";
        return 2;
    }

    try {
        const auto expected = read_expected_fragments(argv[1]);
        const std::string scenario = argc == 3 ? argv[2] : "merge";
        ...
        config.fragmentation_method = 3;
        config.max_cross_length = 5;
        ...
        if (scenario == "merge") {
            append(tracker.process_slice(30, 30000, 1030, slice_with({5, 6, 11}), magnitudes()));
            append(tracker.process_slice(31, 30064, 1031, slice_with({5, 6, 11}), magnitudes()));
            append(tracker.process_slice(32, 30128, 1032, slice_with({5, 6, 7, 8, 9, 10, 11}), magnitudes()));
            append(tracker.process_slice(33, 30192, 1033, slice_with({5, 6, 7, 8, 9, 10, 11}), magnitudes()));
            append(tracker.process_slice(34, 30256, 1034, slice_with({}), magnitudes()));
        }
        else if (scenario == "split") {
            append(tracker.process_slice(30, 30000, 1030, slice_with({5, 6, 7, 8, 9, 10, 11}), magnitudes()));
            append(tracker.process_slice(31, 30064, 1031, slice_with({5, 6, 7, 8, 9, 10, 11}), magnitudes()));
            append(tracker.process_slice(32, 30128, 1032, slice_with({5, 6, 11}), magnitudes()));
            append(tracker.process_slice(33, 30192, 1033, slice_with({5, 6, 11}), magnitudes()));
            append(tracker.process_slice(34, 30256, 1034, slice_with({}), magnitudes()));
        }
        else if (scenario == "cross") {
            append(tracker.process_slice(30, 30000, 1030, slice_with({4, 12}), magnitudes()));
            append(tracker.process_slice(31, 30064, 1031, slice_with({5, 11}), magnitudes()));
            append(tracker.process_slice(32, 30128, 1032, slice_with({5, 6, 7, 8, 9, 10, 11}), magnitudes()));
            append(tracker.process_slice(33, 30192, 1033, slice_with({5, 11}), magnitudes()));
            append(tracker.process_slice(34, 30256, 1034, slice_with({4, 12}), magnitudes()));
            append(tracker.process_slice(35, 30320, 1035, slice_with({}), magnitudes()));
        }
        else {
            throw std::invalid_argument("unknown rejoin fixture scenario: " + scenario);
        }

        const auto actual = fragment_info(completed);
        if (actual != expected) {
            std::cerr << "Connected region rejoin fragmentation parity failed for scenario " << scenario << "\n";
            std::cerr << "expected=" << expected << "\n";
            std::cerr << "actual  =" << actual << "\n";
            return 1;
        }

        std::cout << "Connected region rejoin fragmentation parity passed\n";
        return 0;
    }
