...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 24) {
        throw "Health endpoint did not report result schema version 24"
    }
    ...
    if ($first.schemaVersion -ne 24) {
        throw "PCM response did not report result schema version 24"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 24) {
        throw "Archived result record did not preserve result schema version 24"
    }
