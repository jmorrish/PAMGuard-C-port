...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 11) {
        throw "Health endpoint did not report result schema version 11"
    }
    ...
    if ($first.schemaVersion -ne 11) {
        throw "PCM response did not report result schema version 11"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 11) {
        throw "Archived result record did not preserve result schema version 11"
    }
