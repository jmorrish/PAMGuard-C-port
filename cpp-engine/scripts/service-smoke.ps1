...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 8) {
        throw "Health endpoint did not report result schema version 8"
    }
    ...
    if ($first.schemaVersion -ne 8) {
        throw "PCM response did not report result schema version 8"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 8) {
        throw "Archived result record did not preserve result schema version 8"
    }
