...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 22) {
        throw "Health endpoint did not report result schema version 22"
    }
    ...
    if ($first.schemaVersion -ne 22) {
        throw "PCM response did not report result schema version 22"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 22) {
        throw "Archived result record did not preserve result schema version 22"
    }
