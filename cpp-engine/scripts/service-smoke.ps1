...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 23) {
        throw "Health endpoint did not report result schema version 23"
    }
    ...
    if ($first.schemaVersion -ne 23) {
        throw "PCM response did not report result schema version 23"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 23) {
        throw "Archived result record did not preserve result schema version 23"
    }
