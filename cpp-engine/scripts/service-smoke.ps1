...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 13) {
        throw "Health endpoint did not report result schema version 13"
    }
    ...
    if ($first.schemaVersion -ne 13) {
        throw "PCM response did not report result schema version 13"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 13) {
        throw "Archived result record did not preserve result schema version 13"
    }
