...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 14) {
        throw "Health endpoint did not report result schema version 14"
    }
    ...
    if ($first.schemaVersion -ne 14) {
        throw "PCM response did not report result schema version 14"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 14) {
        throw "Archived result record did not preserve result schema version 14"
    }
