...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 12) {
        throw "Health endpoint did not report result schema version 12"
    }
    ...
    if ($first.schemaVersion -ne 12) {
        throw "PCM response did not report result schema version 12"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 12) {
        throw "Archived result record did not preserve result schema version 12"
    }
