...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 26) {
        throw "Health endpoint did not report result schema version 26"
    }
    ...
    if ($first.schemaVersion -ne 26) {
        throw "PCM response did not report result schema version 26"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 26) {
        throw "Archived result record did not preserve result schema version 26"
    }
