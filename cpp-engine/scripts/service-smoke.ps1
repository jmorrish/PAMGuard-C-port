...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 10) {
        throw "Health endpoint did not report result schema version 10"
    }
    ...
    if ($first.schemaVersion -ne 10) {
        throw "PCM response did not report result schema version 10"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 10) {
        throw "Archived result record did not preserve result schema version 10"
    }
