...
try {
    ...
    if ($lastHealth.resultSchemaVersion -ne 25) {
        throw "Health endpoint did not report result schema version 25"
    }
    ...
    if ($first.schemaVersion -ne 25) {
        throw "PCM response did not report result schema version 25"
    }
    ...
    if ($fullArchive.records[0].schemaVersion -ne 25) {
        throw "Archived result record did not preserve result schema version 25"
    }
