...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\click-train\spectrum-templates.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\SpectrumTemplateFixtureExporter.java"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.SpectrumTemplateFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
