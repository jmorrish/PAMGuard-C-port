...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\whistle\whistle-moan-settings-defaults.json"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\WhistleMoanSettingsFixtureExporter.java"
...
$JavacArgFile = Join-Path $BuildDir "javac-args-whistle-moan-settings.txt"
$JavacErrFile = Join-Path $BuildDir "javac-err-whistle-moan-settings.txt"
@(
    "-nowarn"
    "-cp"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" `
    org.pamguard.port.reference.WhistleMoanSettingsFixtureExporter `
