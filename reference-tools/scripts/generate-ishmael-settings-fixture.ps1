...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\ishmael\ishmael-settings.json"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\IshmaelSettingsFixtureExporter.java"
...
$JavacArgFile = Join-Path $BuildDir "javac-args-ishmael-settings.txt"
$JavacErrFile = Join-Path $BuildDir "javac-err-ishmael-settings.txt"
@(
    "-nowarn"
    "-cp"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" `
    org.pamguard.port.reference.IshmaelSettingsFixtureExporter `
