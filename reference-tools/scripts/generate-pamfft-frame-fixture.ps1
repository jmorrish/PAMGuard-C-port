...
& $java -cp "$buildDir;$fullClasspath" org.pamguard.port.reference.PamFftFrameFixtureExporter $WindowType $FftLength $FftHop $SampleRate $ChunkLength | Set-Content -Path $OutputPath -Encoding UTF8
if ($LASTEXITCODE -ne 0) {
