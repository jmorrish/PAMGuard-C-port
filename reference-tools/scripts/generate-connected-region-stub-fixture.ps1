...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\whistle\connected-region-stub-removal.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\ConnectedRegionStubFixtureExporter.java"
...
& $Java -cp $BuildDir org.pamguard.port.reference.ConnectedRegionStubFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
