...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\click-train\standard-mht-chi2.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\StandardMhtChi2FixtureExporter.java"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.StandardMhtChi2FixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
