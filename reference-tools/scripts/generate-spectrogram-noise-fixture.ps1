...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\whistle\spectrogram-noise.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\SpectrogramNoiseFixtureExporter.java"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.SpectrogramNoiseFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
