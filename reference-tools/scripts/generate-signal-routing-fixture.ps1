...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\signal-routing\signal-routing.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\SignalRoutingFixtureExporter.java"
...
$javacArgFile = Join-Path $BuildDir "javac-args-signal-routing.txt"
$javacErrFile = Join-Path $BuildDir "javac-err-signal-routing.txt"
@(
    "-nowarn"
    "-cp"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.SignalRoutingFixtureExporter $Output
