...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\noise-ltsa\settings-defaults.json"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\NoiseLtsaSettingsFixtureExporter.java"
...
$JavacArgFile = Join-Path $BuildDir "javac-args-noise-ltsa-settings.txt"
$JavacErrFile = Join-Path $BuildDir "javac-err-noise-ltsa-settings.txt"
@(
    "-nowarn"
    "-cp"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" `
    org.pamguard.port.reference.NoiseLtsaSettingsFixtureExporter `
