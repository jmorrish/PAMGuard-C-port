...
$OracleEnvironment = & (Join-Path $ScriptDir "resolve-pamguard-oracle.ps1") -PortRoot $PortRoot -RequireClasses
...
#                 sampleRate totalLength output scenario [processChunkLength]
$Fixtures = @(
    ...
    @{ Name = "boundary-transient-chunk128.csv";   Args = @("0x3", "0x3", "10.0", "0.1", "0.00001", "10", "12", "8", "128", "1", "48000", "256", "boundary-transient"); ProcessChunkLength = "128" }
)

foreach ($Fixture in $Fixtures) {
    $Output = Join-Path $FixtureDir $Fixture.Name
    $ExporterArgs = @($Fixture.Args[0..11]) + @($Output, $Fixture.Args[12])
    if ($Fixture.ProcessChunkLength) {
        $ExporterArgs += $Fixture.ProcessChunkLength
    }
