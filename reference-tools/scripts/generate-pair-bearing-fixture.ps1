...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\localisation\pair-bearing.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\PairBearingFixtureExporter.java"
...
& $Java -cp $BuildDir org.pamguard.port.reference.PairBearingFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
