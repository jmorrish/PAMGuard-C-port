...
$FixtureDir = Join-Path $PortRoot "cpp-engine\tests\fixtures\click-classifier"
$ClassificationOutput = Join-Path $FixtureDir "sweep-click-classifier.csv"
$DefaultsOutput = Join-Path $FixtureDir "sweep-click-defaults.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\SweepClassifierFixtureExporter.java"
...
& $Oracle.Java -cp "$BuildDir;$FullClasspath" `
    org.pamguard.port.reference.SweepClassifierFixtureExporter `
    $ClassificationOutput $DefaultsOutput
