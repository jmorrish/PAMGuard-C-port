...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\click-classifier\basic-click-classifier.csv"
$JavaHome = if ($env:JAVA_HOME) { $env:JAVA_HOME } else { "C:\Program Files\Eclipse Adoptium\jdk-21.0.11.10-hotspot" }
$Java = Join-Path $JavaHome "bin\java.exe"
$Javac = Join-Path $JavaHome "bin\javac.exe"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\ClickBasicClassifierFixtureExporter.java"
...
& $Java -cp "$BuildDir;$FullClasspath" org.pamguard.port.reference.ClickBasicClassifierFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
