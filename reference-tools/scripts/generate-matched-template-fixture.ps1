...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\matched-template\matched-template.csv"
$JavaHome = if ($env:JAVA_HOME) { $env:JAVA_HOME } else { "C:\Program Files\Eclipse Adoptium\jdk-21.0.11.10-hotspot" }
$Java = Join-Path $JavaHome "bin\java.exe"
$Javac = Join-Path $JavaHome "bin\javac.exe"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\MatchedTemplateFixtureExporter.java"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.MatchedTemplateFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
