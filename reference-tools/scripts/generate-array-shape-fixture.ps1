...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\localisation\array-shape.csv"
$JavaSrc = Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\ArrayShapeFixtureExporter.java"
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.ArrayShapeFixtureExporter $Output
if ($LASTEXITCODE -ne 0) {
