...
$Output = Join-Path $PortRoot "cpp-engine\tests\fixtures\localisation\ml-line-bearing.csv"
...
$JavaSrc = @(
    (Join-Path $PortRoot "reference-tools\java\src\org\pamguard\port\reference\MlGridBearingFixtureExporter.java"),
...
& $Java -cp "$BuildDir;$TargetClasses;$DependencyClasspath" org.pamguard.port.reference.MlGridBearingFixtureExporter $Output line
if ($LASTEXITCODE -ne 0) {
