...
$FixtureDir = Join-Path $PortRoot "cpp-engine\tests\fixtures\grouped-source"
$Output = Join-Path $FixtureDir "grouped-source.csv"
...
$Source = Join-Path $PortRoot `
    "reference-tools\java\src\org\pamguard\port\reference\GroupedSourceFixtureExporter.java"
& $Oracle.Javac -cp $Classpath -d $BuildDir $Source
if ($LASTEXITCODE -ne 0) { throw "javac failed with exit code $LASTEXITCODE" }
& $Oracle.Java -cp "$BuildDir;$Classpath" `
    org.pamguard.port.reference.GroupedSourceFixtureExporter $Output
