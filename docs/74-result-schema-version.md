...
- `26`: Adds a `matchedTemplateClassifications` array (ported PAMGuard matched-template click classifier): per click, the best match/reject correlation result per template pair across channels (`threshold`, `matchCorr`, `rejectCorr` — omitted when the reject template yields NaN) and the aggregated `classified` flag. The compatibility endpoint now uses the same strict canonical object as the active project: `matchedTemplate: {enabled, clickType, normalisationType, peakSearch, peakSmoothing, lengthDb, restrictedBins, channelClassification, classifiers: [{thresholdToAccept, normalisation, matchTemplate: {name, sampleRateHz, waveform}, rejectTemplate: {...}}]}`. Old `match`/`reject` aliases are intentionally rejected. Template rates above the session rate use the separately pinned `WavInterpolator` decimation port.
