...
- Port the remaining MHT pieces: the correlation chi2 variable (needs PAMGuard's waveform CorrelationManager), wiring time-delay/correlation variables into the served stack, and the rest of the click train classifier chain (template/standard classifiers and CTClassifierManager chaining, `docs/166`-`docs/178`).
