...
- **A GPS and attitude feed.** Two remaining items collapse into this one missing input:
  - time-varying streamer positions and array attitude for towed arrays (`docs/190`, `docs/193`, `docs/202`);
  - anything that would make `DetectionGroupLocaliser` meaningful (`docs/192`).

  Static geometry, static attitude, array-frame vectors, and **static earth-frame vectors** (`docs/202`) are all ported and served; what needs the feed is *time-varying* orientation and geographic position, not reference maths.
