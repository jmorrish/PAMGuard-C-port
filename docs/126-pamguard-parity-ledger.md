...
| Pair bearing localiser | Fixture parity | Seven-case Java fixture for angle/error including endfire clamp, negative spacing, and three-delay reduction (`docs/157-pair-bearing-localiser-port.md`) | `prepare()`-side array derivation is config-supplied; not yet exposed via the service. |
| LSQ bearing localiser | Fixture parity | Four-case fixture generated by real Jama QR/PamVector; rank-deficiency semantics pinned (`docs/158-lsq-bearing-localiser-port.md`) | Needs at least four non-coplanar hydrophones by construction; MLGrid/simplex localisers unported; not yet exposed via the service. |
...
- Wire pair/LSQ bearing localiser selection into the click pipeline and service output (schema-versioned change).
