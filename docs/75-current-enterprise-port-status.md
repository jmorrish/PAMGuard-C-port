...
- `ctest` passes `52/52` tests on this Windows build, including registered service smoke tests, API-key-file smoke coverage, FFmpeg ingest help coverage, ingest supervisor status/command smoke coverage, archive event index rebuild smoke coverage, and grouped archive retention smoke coverage.
