...
Closes the ledger's largest Gap to the extent it can be closed without external input. `docs/182` established that PAMGuard project import must run on the JVM (the payload is Java-serialised object graphs) and was recorded as blocked on "a pinned PAMGuard version and a `.psfx` written by it". The work in this slice used PAMGuard `2.02.18b`, whose `PSFXReadWriter.writePSFX(String, PamSettingsGroup)` takes an explicit settings group, so a genuine `.psfx` could be written headlessly by that version itself.
...
| `AngleVetoParameters` + `AngleVeto` | `click.angleVetoes[]`: stored channel bitmap and inclusive start/end angles |
| `NoiseSettings` + `NoiseMeasurementBand` | `fftNoise`: channels, interval, sampling policy, and named frequency bands |
...
| `WhistleToneParameters` | `whistle`: audio-channel bitmap/grouping, frequency limits in Hz, background snapshot interval, min pixels/length, max cross length, connect type, fragmentation method, shape stubs, and the complete spectrogram-noise settings chain. The legacy BetterPeak output is disabled because Whistles & Moans does not consume it (`docs/231`, `docs/233`, `docs/234`). |

Whistle settings without FFT source settings are reported as unmappable rather than guessed.
...
**The original evidence was pinned to `2.02.18b`; it has now been regenerated and revalidated against `2.02.18e`** at the exact commit recorded in `reference-tools/pamguard-oracle.json`; see `docs/225-java-oracle-pinning.md`. The new oracle wrote the sample `.psfx` and converted it successfully, preserving the session JSON surface. A `.psfx` from any different PAMGuard build may fail to deserialise (exit 3), and that brittleness is a property of Java serialisation PAMGuard itself lives with.
